...
with st.sidebar:
    ...
    if job_type == "Import (Delivery)":
        ...
        def get_export_mat_type(m):
            ml = m.lower()
            if 'concrete' in ml: return 'Concrete'
            if 'asphalt' in ml: return 'Asphalt'
            if 'topsoil' in ml: return 'Topsoil'
            if 'fill' in ml or 'dirt' in ml: return 'Dirt / Fill'
            if 'waste' in ml or 'debris' in ml: return 'Waste / Debris'
            return 'Other'

        # Predefined list for export if dumps are empty, otherwise from dumps
        export_defaults = ["Clean Fill", "Topsoil (Unscreened)", "Topsoil (Screened)", "Concrete (Clean)", "Concrete (Dirty/Reinforced)", "Asphalt (Chunked)", "Asphalt (Milled)", "Construction Waste"]
        dump_mats = sorted(list(set([d['Material'] for d in dumps])))
        all_materials = dump_mats if dump_mats else export_defaults
        
        mat_types = sorted(list(set([get_export_mat_type(m) for m in all_materials])))
        
        selected_type = st.selectbox("Material Type", ["All Types"] + mat_types)
        
        if selected_type != "All Types":
            filtered_by_type = [m for m in all_materials if get_export_mat_type(m) == selected_type]
        else:
            filtered_by_type = all_materials
    ...
    with st.expander("⚙️ **Trucking Parameters**", expanded=False):
        ...
        min_hours_per_truck = st.number_input("Minimum Hours per Truck", min_value=0.0, max_value=12.0, value=default_min_hr, step=0.5)
